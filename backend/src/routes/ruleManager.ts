...
// Follow the way I define api request handling in server.ts. Make changes in server.ts yourself.
// Do not edit the database on your own,
// use utility functions from my db.ts. If you need to add such functions ask me
// or add them yourself the same way they are defined in db.ts
...
router.get("/:id", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM fake_api_rules WHERE id = $1",
      [req.params.id]
    );
...
router.delete("/:id", async (req, res) => {
  try {
    const result = await pool.query(
      "DELETE FROM fake_api_rules WHERE id = $1 RETURNING *",
      [req.params.id]
    );
    if (result.rowCount === 0) return res.status(404).send("Not found");
    res.status(204).send();
